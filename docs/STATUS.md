...
- A dirty-tree p17 diagnostic at `T=27/50` over `N=80,84,...,144` reached generator-side `CANDIDATE_READY` at `N=104`: the refined scout first became stable-positive at `N=100`, rigorous full-tail screening classified `N=100` as mathematical-negative after the full 128/256/384/512-bit ladder, `N=104` stabilized positive at 512 bits, and the 64-bit-matrix / 32-bit-witness exact candidate remained positive under 640-bit fixed-parameter reassembly. The diagnostic bundle passed its 19-artifact hash/size audit and recorded complete worker cleanup, but its manifest correctly has `git_dirty=true`; it is therefore tooling/diagnostic evidence only, not a canonical retained computation, theorem certificate, admission, or frontier advance. The next research action remains a clean-tree canonical `T=27/50` rerun after p17 closure.
