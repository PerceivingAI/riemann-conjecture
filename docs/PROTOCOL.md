...
## 12. Certificate verification and audit protocol

Every numerical claim supporting a derived finding or positivity statement must follow the certificate verification protocol:

1. **Artifact Generation**: The claim must be generated by Python using verified interval enclosures (`scripts/cert/`) and exported as a standardized JSON certificate conforming to `rh-weil-certificate-v1` (`docs/contracts/rh-weil-certificate-v1.json`).
2. **Storage Location**: The generated certificate must be stored in the computation bundle data directory:
   ```text
   computations/YYYY-MM-DDTHHMMSSZ-<title>/data/certificate.json
   ```
3. **Independent Verification Gate**: Before any finding based on numerical positivity is transitioned to `VERIFIED` status in `CLAIMS.md` or `findings/`, the certificate must pass independent zero-float verification via the Rust verifier:
   ```bash
