...
The closed certificate syntax and exact PASS semantics are authoritative in [`docs/CONTRACTS.md`](docs/CONTRACTS.md) and [`docs/contracts/rh-weil-certificate-v1.json`](docs/contracts/rh-weil-certificate-v1.json). The v1 `exact_prime_legendre_schur` profile is a closed whitelist, currently admitting exactly `(T,N)=(7/20,32)`, `(2/5,40)`, `(17/40,48)`, `(9/20,56)`, `(19/40,68)`, `(1/2,80)`, and `(21/40,96)`. For each admitted pair it derives the Legendre complement bound, reconstructs the factor-3 component-Gram Schur matrix, and verifies exact rational congruence/Gershgorin witnesses. Fresh independently replayed certificates establish `C-0050` through `C-0056` as localized finite-support theorems; the profile does not accept arbitrary nearby supports such as `(21/40,100)`.
...
Retention adds an additional audit layer without changing those theorem semantics: `computations/retained-proofs.json` explicitly names the seven proof-bearing certificate files and their expected hashes/theorem identities, while `scripts.cert.verify_retained_proofs` verifies that those exact stored artifacts remain intact and are still accepted by the current independent verifier.
