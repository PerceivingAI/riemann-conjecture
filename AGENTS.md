...
Current provenance versioning: `continuation-driver-p17-v1` is the canonical workflow version and `continuation-driver-v6` is the cache contract. P17 retains the p16 lifecycle/observability contract but strengthens floating reconnaissance: the canonical scout uses eight increasing resolution levels derived from the requested maximum dimension, retains every level in the bundle, and classifies convergence/sign stability from the highest three levels under the existing 1% relative Schur-movement tolerance. This prevents an under-resolved coarse level from poisoning an otherwise converged scout while leaving the rigorous Arb full-tail stage as the mathematical gate. Treat `driver_version` as the version of the full canonical workflow contract—including scout selection semantics, run identity, observability, worker cleanup, completion ordering, and transactional finalization—not merely the final mathematical payload. Treat `cache_version` as the narrower cached mathematical payload/key contract. The cache remains v6; source fingerprints already isolate changed continuation implementations. Atomic publication of live JSON, bundle artifacts, and cache entries uses bounded retry for transient `PermissionError`/Windows sharing denials and still fails closed if the retry budget is exhausted.
