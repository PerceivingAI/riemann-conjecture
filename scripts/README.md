...
Finalization is transactional. Reaching a mathematical terminal state does not by itself create a completed bundle. The driver first exits every process-pool context, verifies that each captured worker process is no longer alive and has an exit code, verifies that no executor remains active, and only then freezes the final result through JSON and records its SHA-256. Stage artifacts are written first, `summary.json` is written after them, and `run-manifest.json` is written **last** as the bundle completion seal. The manifest records the verified worker-cleanup summary and frozen-result digest. Only after that manifest write succeeds does live status become `terminal=true`, followed by the `RUN_COMPLETED` event; the heartbeat is joined and the output-directory lock is then released before final stdout is emitted. Therefore a valid `run-manifest.json` means the mathematical result had already been reached and all parallel worker cleanup had already been verified, not merely that computation stopped producing output.
